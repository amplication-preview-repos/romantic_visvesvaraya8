/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { LobbiesUpdateManyWithoutQuizPackagesItemsInput } from "./LobbiesUpdateManyWithoutQuizPackagesItemsInput";
import { Type } from "class-transformer";
import { TopicsUpdateManyWithoutQuizPackagesItemsInput } from "./TopicsUpdateManyWithoutQuizPackagesItemsInput";

@InputType()
class QuizPackagesUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => LobbiesUpdateManyWithoutQuizPackagesItemsInput,
  })
  @ValidateNested()
  @Type(() => LobbiesUpdateManyWithoutQuizPackagesItemsInput)
  @IsOptional()
  @Field(() => LobbiesUpdateManyWithoutQuizPackagesItemsInput, {
    nullable: true,
  })
  lobbiesItems?: LobbiesUpdateManyWithoutQuizPackagesItemsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => TopicsUpdateManyWithoutQuizPackagesItemsInput,
  })
  @ValidateNested()
  @Type(() => TopicsUpdateManyWithoutQuizPackagesItemsInput)
  @IsOptional()
  @Field(() => TopicsUpdateManyWithoutQuizPackagesItemsInput, {
    nullable: true,
  })
  topicsItems?: TopicsUpdateManyWithoutQuizPackagesItemsInput;
}

export { QuizPackagesUpdateInput as QuizPackagesUpdateInput };
