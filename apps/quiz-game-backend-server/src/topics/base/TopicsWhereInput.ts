/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { StringNullableFilter } from "../../util/StringNullableFilter";
import { Type } from "class-transformer";
import { IsOptional, ValidateNested } from "class-validator";
import { StringFilter } from "../../util/StringFilter";
import { QuestionsListRelationFilter } from "../../questions/base/QuestionsListRelationFilter";
import { QuizPackagesWhereUniqueInput } from "../../quizPackages/base/QuizPackagesWhereUniqueInput";

@InputType()
class TopicsWhereInput {
  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  description?: StringNullableFilter;

  @ApiProperty({
    required: false,
    type: StringFilter,
  })
  @Type(() => StringFilter)
  @IsOptional()
  @Field(() => StringFilter, {
    nullable: true,
  })
  id?: StringFilter;

  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  name?: StringNullableFilter;

  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  packageId?: StringNullableFilter;

  @ApiProperty({
    required: false,
    type: () => QuestionsListRelationFilter,
  })
  @ValidateNested()
  @Type(() => QuestionsListRelationFilter)
  @IsOptional()
  @Field(() => QuestionsListRelationFilter, {
    nullable: true,
  })
  questionsItems?: QuestionsListRelationFilter;

  @ApiProperty({
    required: false,
    type: () => QuizPackagesWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QuizPackagesWhereUniqueInput)
  @IsOptional()
  @Field(() => QuizPackagesWhereUniqueInput, {
    nullable: true,
  })
  quizPackage?: QuizPackagesWhereUniqueInput;

  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  topicQuizPackage?: StringNullableFilter;
}

export { TopicsWhereInput as TopicsWhereInput };
