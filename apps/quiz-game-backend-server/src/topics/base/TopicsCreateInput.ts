/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { QuestionsCreateNestedManyWithoutTopicsItemsInput } from "./QuestionsCreateNestedManyWithoutTopicsItemsInput";
import { Type } from "class-transformer";
import { QuizPackagesWhereUniqueInput } from "../../quizPackages/base/QuizPackagesWhereUniqueInput";

@InputType()
class TopicsCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  packageId?: string | null;

  @ApiProperty({
    required: false,
    type: () => QuestionsCreateNestedManyWithoutTopicsItemsInput,
  })
  @ValidateNested()
  @Type(() => QuestionsCreateNestedManyWithoutTopicsItemsInput)
  @IsOptional()
  @Field(() => QuestionsCreateNestedManyWithoutTopicsItemsInput, {
    nullable: true,
  })
  questionsItems?: QuestionsCreateNestedManyWithoutTopicsItemsInput;

  @ApiProperty({
    required: false,
    type: () => QuizPackagesWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => QuizPackagesWhereUniqueInput)
  @IsOptional()
  @Field(() => QuizPackagesWhereUniqueInput, {
    nullable: true,
  })
  quizPackage?: QuizPackagesWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  topicQuizPackage?: string | null;
}

export { TopicsCreateInput as TopicsCreateInput };
